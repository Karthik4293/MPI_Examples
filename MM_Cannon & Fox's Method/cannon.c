...
void main (int argc, char *argv[])
{
 ...
 int rotation, addition, multiplication;
 ...
 double t2, t1, t3, time_P, total;
 double norm = pow(10,9);
 ...
 /* Reading Input at the root processor */
  if (grid.MyRank == Root){
     if ((fp = fopen ("./data/mdata1.txt", "r")) == NULL){
  ...
  fclose(fp);
     ...
     if((fp = fopen ("./data/mdata2.txt", "r")) == NULL){
 ...
 /* The main loop ----  Calculating the local product of matrix C (local indices used) */
 ...
 /* initial arrangement of the matrices */
 ...
 /* Memory for output global array for OutputMatrix_C after rearrangement */
  ...
  int delay = pow(10,2*grid.MyRank);
  t2 = MPI_Wtime();
  time_P = (t2-t1)*t3;


  printf("------------------ rank : %d------------\n", grid.MyRank );
  printf("Time taken: %1.3f ns\n", time_P*norm);
  printf("Rotations : %d\n", rotation);
  printf("Additions : %d\n", addition);
  ...
  if (grid.MyRank == Root) {
    for (int R=1;R<grid.Size;R++){
      MPI_Recv(&time_P,sizeof(time_P), MPI_DOUBLE,R,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
      total = total + time_P;
    }
      printf("\n");
      printf("\n");
      printf ("total time for mvm using cannon's method: %2.3f ns\n", total*norm);
      printf("Resolution of tick : %1.3f ns\n", t3*norm);
      printf("Total rotations: %d\n", rotation*(grid.Size));
      printf("Total number of Multiplications: %d\n", multiplication*(grid.Size));
      printf("Total number of Additions: %d\n", addition*(grid.Size));
  }
    else {
      MPI_Send(&time_P,sizeof(time_P),MPI_DOUBLE,0,1,MPI_COMM_WORLD);
    }
